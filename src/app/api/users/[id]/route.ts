...
export async function GET(req: Request, { params }: RouteParams) {
    try {
        ...
        if (currentUser && (currentUser.user_id === userId || currentUser.role === "admin")) {
            selectColumns += ", email, phone, created_at, updated_at"
        }
...
export async function PUT(req: Request, { params }: RouteParams) {
    try {
        ...
        const { name, email, avatar, bio, username, phone } = (await req.json()) as Partial<User>
        ...
        const fields = { name, email, avatar: avatarFinal, bio, username, phone, updated_at: new Date() }
        ...
        const sql = `UPDATE users SET ${setClause} WHERE user_id = $${entries.length + 1} RETURNING user_id, name, username ,email, avatar, bio, phone, updated_at`
